...
BOOL CSplash::Create(CWnd *pWndParent,
	                 LPCTSTR szText,
	                 DWORD dwTimeout,
	                 DWORD dwStyle)
{
	...
	wcx.lpszClassName = _T("SplashClass") ;
	wcx.hIconSm = NULL ;
...
BOOL CSplash::SetBitmap(UINT nBitmapID, short red, short green, short blue)
{
	BITMAP bm ;
...
BOOL CSplash::SetBitmap(LPCTSTR szFileName, short red, short green, short blue)
{
	BITMAP bm ;
	HBITMAP hBmp ;
...
void CSplash::SetTextFont(LPCTSTR szFont, int nSize, int nStyle)
{
	LOGFONT lf ;
...
void CSplash::SetTextDefaultFont()
{
	LOGFONT lf ;
...
void CSplash::SetText(LPCTSTR szText)
{
	m_strText = szText ;
	RedrawWindow() ;
}

void CSplash::SetTextColor(COLORREF crTextColor)
{
	m_crTextColor = crTextColor ;
	RedrawWindow() ;
}

void CSplash::SetTextRect(CRect& rcText)
{
	m_rcText = rcText ;
	RedrawWindow() ;
}

void CSplash::SetTextFormat(UINT uTextFormat)
{
...
HRGN CSplash::CreateRgnFromBitmap(HBITMAP hBmp, COLORREF color)
{
	...
	for ( i = 0; i < bm.bmHeight; i++ )
	{
		for ( j = 0; j < bm.bmWidth; j++ )
		{
			...
			if (!wasfirst && ismask) // set wasfirst when mask is found
			{
				first = j ;
				wasfirst = true ;
			}
...
void CSplash::DrawWindow(CDC *pDC)
{
	CDC memDC ;
	CBitmap* pOldBitmap ;
...
LRESULT CSplash::OnPrintClient(WPARAM wParam, LPARAM lParam)
{
...
void CSplash::OnTimer(UINT_PTR nIDEvent) // for x64
//void CSplash::OnTimer(UINT nIDEvent) // for x86
{
	KillTimer(0) ;
	Hide() ;
}
